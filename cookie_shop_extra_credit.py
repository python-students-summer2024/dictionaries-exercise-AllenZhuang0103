...
def bake_cookies(filepath):
    ...
    with open(filepath, mode='r') as file:
        ...
        for line in file:
            ...
            if parts[4] == 'sugar free':
                suffer_from_diabetes = False
            else:
                suffer_from_diabetes = True
            if parts[4] == 'gluten free':
                need_gluten = False
            else:
                need_gluten = True
            if parts[4] == 'contain nuts':
                need_nuts = True
            else:
                need_nuts = False
            ...
            cookie_data = {
                ...
                'suffer_from_diabetes': suffer_from_diabetes,
                'need_gluten': need_gluten,
                'need_nuts': need_nuts
            }
            if len(cookie_data['title']) < 2 or len(cookie_data['description']) < 10:
                continue  
...
def welcome():
    ...
    while True:
        need_nuts = input("Are you allergic to nuts? (yes/no)").strip().lower()
        need_gluten = input("Are you allergic to gluten? (yes/no)").strip().lower()
        suffer_from_diabetes = input('Do you suffer from diabetes? (yes/no)').strip().lower()
        if need_nuts not in ['yes', 'y', 'no', 'n'] or need_gluten not in ['yes', 'y', 'no', 'n'] or suffer_from_diabetes not in ['yes', 'y', 'no', 'n']:
            print("Please enter a valid response.")
            continue
        return need_nuts, need_gluten, suffer_from_diabetes


def display_cookies(cookies, need_nuts, need_gluten, suffer_from_diabetes):
    """
    ...
    for cookie in cookies:
        if need_nuts == 'yes' and cookie['need_nuts']:
            print(f"  #{cookie['id']} - {cookie['title']}\n  {cookie['description']}\n  Price: ${cookie['price']:.2f}\n")
        elif need_gluten == 'no' and not cookie['need_gluten']:
            print(f"  #{cookie['id']} - {cookie['title']}\n  {cookie['description']}\n  Price: ${cookie['price']:.2f}\n")
        elif suffer_from_diabetes == 'no' and not cookie['suffer_from_diabetes']:
            print(f"  #{cookie['id']} - {cookie['title']}\n  {cookie['description']}\n  Price: ${cookie['price']:.2f}\n")
...
def solicit_order(cookies):
    ...
    order = []
    while True:
        user_input = input("Please enter the number of any cookie you would like to purchase: ")
        if user_input.lower() in ['finished', 'done', 'quit', 'exit']:
            break
        if user_input.isnumeric():
            cookie_id = int(user_input)
            if any(cookie['id'] == cookie_id for cookie in cookies):
...
def display_order_total(order, cookies):
    ...
    for item in order:
        ...
        total += cookie['price'] * item['quantity']
        print(f"-{item['quantity']} {cookie['title']}")
    print(f"Your total is ${total:.2f}. Please pay with Bitcoin before picking-up.")
    print("Thank you! -The Python Cookie Shop Robot.")
...
def run_shop(cookies):
    ...
    """
    # write your code for this function below here.
    need_nuts, need_gluten, suffer_from_diabetes = welcome()
    display_cookies(cookies, need_nuts, need_gluten, suffer_from_diabetes)
